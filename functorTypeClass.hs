...
-- | class Functor takes a type (f) that also take a type types conforming to
-- Functor must implement fmap where fmap takes a function that (may) change type
-- and then an instance of (concrete) type (f a) which returns (f b)
--
-- And (f *) must be a concrete type as it's the value in a function (namely
-- fmap)
--
-- i.e. mapping a list (f) of Int (f a) to a list (f) of String (f b)
...
insert k v (AssocListItem k' v' ls) =
    if k' == k
